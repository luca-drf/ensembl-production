...
sub reformat_core {
  ...
  if (!defined $slices) {
    $slices = [ sort {$b->length() <=> $a->length()}
        @{$dba->get_SliceAdaptor()->fetch_all('toplevel', undef, 1, undef, undef)}
    ];
  }
  ...
  my $slice_trans = Bio::EnsEMBL::IO::Translator::Slice->new(version => $release,
  ...
  my $feature_trans = Bio::EnsEMBL::IO::Translator::BulkFetcherFeature->new(
      ...
      adaptor           => $dba, # required for mapping Ensembl things to RDF
  ...
  $core_writer->write(
      Bio::EnsEMBL::IO::Object::RDF->dataset(
          version         => $release,
          project         => $meta_adaptor->get_division() || 'EnsemblVertebrates',
...
sub reformat_xrefs {
  ...
  my $feature_trans = Bio::EnsEMBL::IO::Translator::BulkFetcherFeature->new(
      version           => $release,
      xref_mapping_file => $self->{config_file}, # required for mapping Ensembl things to RDF
      adaptor           => $dba,
      ontology_adaptor  => $self->{onto_dba}->get_OntologyTermAdaptor(),
